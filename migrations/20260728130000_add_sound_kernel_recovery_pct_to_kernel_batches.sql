-- Add a per-batch "sound kernel recovery %" to get_kernel_batches, for the Find a batch (batch
-- journey) Kernel list: total sound-style kernel produced (SP, 0, 1, 1S, 4L, 5, 6, 7/8 — including
-- 7/8, unlike the company-wide dashboard KPI) divided by the batch's pre-presizer received weight
-- (actual_wet_nis_kg), as a percentage. Only calculated once production is finished
-- (production_finished_at IS NOT NULL); NULL otherwise so the frontend can show a dash.
--
-- Implemented as a thin wrapper around the existing get_kernel_batches query (left byte-for-byte
-- unchanged as a subquery) so the already-correct yield_by_style computation (job-card vs.
-- packing_data fallback) isn't duplicated or risked.
