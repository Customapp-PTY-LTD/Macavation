-- Dashboard "Kernel batches in production" count should match Kernel Production grid.
-- Previously: counted only batches NOT IN ('complete', 'in_finished_stock'), so the card showed fewer than the grid.
-- Now: count all active kernel batches (same as get_kernel_batches with no status filter).
...
BEGIN
    -- Count all active kernel batches (same set as Kernel Production grid / get_kernel_batches with no status filter).
