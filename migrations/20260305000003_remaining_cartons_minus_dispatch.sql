-- Fix remaining_by_style_cartons: subtract dispatched cartons (from kernel_dispatch_orders.lines).
-- Available cartons = yield cartons - sum(cartons taken out per style). Uses le->>'cartons'; if missing, uses quantity_kg/11.34.
...
BEGIN
    ...
    SELECT
        ...
        -- remaining_by_style_cartons = yield cartons - dispatched cartons (use cartons; fallback quantity_kg/11.34)
        (
            SELECT jsonb_build_object(
                'SP',              GREATEST(0, COALESCE(yc.sp,  0) - COALESCE(dc.sp,  0)),
                '0',               GREATEST(0, COALESCE(yc.s0,  0) - COALESCE(dc.s0,  0)),
                '1',               GREATEST(0, COALESCE(yc.s1,  0) - COALESCE(dc.s1,  0)),
                '1S',              GREATEST(0, COALESCE(yc.s1s, 0) - COALESCE(dc.s1s, 0)),
                '4L',              GREATEST(0, COALESCE(yc.s4l, 0) - COALESCE(dc.s4l, 0)),
                '5',               GREATEST(0, COALESCE(yc.s5,  0) - COALESCE(dc.s5,  0)),
                '6',               GREATEST(0, COALESCE(yc.s6,  0) - COALESCE(dc.s6,  0)),
                '7/8',             GREATEST(0, COALESCE(yc.s78, 0) - COALESCE(dc.s78, 0)),
                'Butter High Oil', GREATEST(0, COALESCE(yc.bh,  0) - COALESCE(dc.bh,  0)),
                'Butter Low Oil',  GREATEST(0, COALESCE(yc.bl,  0) - COALESCE(dc.bl,  0))
            )
            FROM (
                SELECT
                    COALESCE(SUM(NULLIF(e ->> 'sk_sp_cartons',  '')::numeric), 0) AS sp,
                    COALESCE(SUM(NULLIF(e ->> 'sk_0_cartons',   '')::numeric), 0) AS s0,
                    COALESCE(SUM(NULLIF(e ->> 'sk_1_cartons',   '')::numeric), 0) AS s1,
                    COALESCE(SUM(NULLIF(e ->> 'sk_1s_cartons',  '')::numeric), 0) AS s1s,
                    COALESCE(SUM(NULLIF(e ->> 'sk_4l_cartons',  '')::numeric), 0) AS s4l,
                    COALESCE(SUM(NULLIF(e ->> 'sk_5_cartons',   '')::numeric), 0) AS s5,
                    COALESCE(SUM(NULLIF(e ->> 'sk_6_cartons',   '')::numeric), 0) AS s6,
                    COALESCE(SUM(NULLIF(e ->> 'bt_78_cartons',  '')::numeric), 0) AS s78,
                    COALESCE(SUM(NULLIF(e ->> 'bt_high_cartons','')::numeric), 0) AS bh,
                    COALESCE(SUM(NULLIF(e ->> 'bt_low_cartons', '')::numeric), 0) AS bl
                FROM jsonb_array_elements(
                    COALESCE(NULLIF(k.packing_data, 'null'::jsonb), '[]'::jsonb)
                ) e
            ) yc
