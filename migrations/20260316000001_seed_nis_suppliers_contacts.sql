-- Seed NIS Suppliers into contacts so they appear in Contact Database Management > NIS Suppliers tab.
-- Source: Macadamia Kernel Statistics '25, Suppliers sheet (docs/NIS_suppliers.json).
-- Idempotent: only inserts if no contact with same company_name and contact_type = 'nis_supplier' exists.
