-- Food grade vs cosmetic oil stream when starting an oil bin batch.
...
-- Replace start_oil_bin_batch (add p_oil_stream)
...
DECLARE
    ...
    v_stream       varchar;
BEGIN
    v_stream := lower(trim(COALESCE(p_oil_stream, '')));
    IF v_stream = '' THEN
        v_stream := NULL;
...
-- get_oil_bin_batches: add oil_stream
...
-- update_oil_bin_batch: allow editing oil_stream
...
-- send_oil_bin_batch_to_stock: include oil_stream on oil.production_data
