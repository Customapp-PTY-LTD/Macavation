-- Batch History: ensure get_kernel_production_history returns dispatch_orders (some DBs never got
-- 20260515180000 / 20260516120000). Match lines by kernel.id, by batches.id wrongly stored as kernel_id,
-- or by batch_number when kernel_id is empty.
    ...
    SELECT
        ...
        (
                ...
                jsonb_agg(
                    jsonb_build_object(
                        ...
                        'lines', (
                            ...
                            WHERE (
                                NULLIF(trim(both FROM COALESCE(le.e->>'kernel_id', le.e->>'KernelId', '')), '') IS NOT NULL
                                AND NULLIF(trim(both FROM COALESCE(le.e->>'kernel_id', le.e->>'KernelId', '')), '')::uuid = p_kernel_id
                            )
                            OR (
                                NULLIF(trim(both FROM COALESCE(le.e->>'kernel_id', le.e->>'KernelId', '')), '') IS NOT NULL
                                AND NULLIF(trim(both FROM COALESCE(le.e->>'kernel_id', le.e->>'KernelId', '')), '')::uuid = k.batch_id
                            )
                            OR (
                                NULLIF(trim(both FROM COALESCE(le.e->>'kernel_id', le.e->>'KernelId', '')), '') IS NULL
                                AND NULLIF(trim(both FROM COALESCE(le.e->>'batch_number', le.e->>'BatchNumber', '')), '') IS NOT NULL
                                AND trim(both FROM COALESCE(le.e->>'batch_number', le.e->>'BatchNumber', '')) = trim(both FROM b.batch_id)
                            )
                        )
                    )
            ...
            WHERE EXISTS (
                ...
                WHERE (
                    NULLIF(trim(both FROM COALESCE(ex.x->>'kernel_id', ex.x->>'KernelId', '')), '') IS NOT NULL
                    AND NULLIF(trim(both FROM COALESCE(ex.x->>'kernel_id', ex.x->>'KernelId', '')), '')::uuid = p_kernel_id
                )
                OR (
                    NULLIF(trim(both FROM COALESCE(ex.x->>'kernel_id', ex.x->>'KernelId', '')), '') IS NOT NULL
                    AND NULLIF(trim(both FROM COALESCE(ex.x->>'kernel_id', ex.x->>'KernelId', '')), '')::uuid = k.batch_id
                )
                OR (
                    NULLIF(trim(both FROM COALESCE(ex.x->>'kernel_id', ex.x->>'KernelId', '')), '') IS NULL
                    AND NULLIF(trim(both FROM COALESCE(ex.x->>'batch_number', ex.x->>'BatchNumber', '')), '') IS NOT NULL
                    AND trim(both FROM COALESCE(ex.x->>'batch_number', ex.x->>'BatchNumber', '')) = trim(both FROM b.batch_id)
                )
            )
        ) AS dispatch_orders
...
'Kernel batch history for Batch History modal: intake, stages, job card, QA, dispatch_orders (lines matched by kernel.id, by batches.id wrongly stored as kernel_id, or by batch_number when kernel_id empty).';
