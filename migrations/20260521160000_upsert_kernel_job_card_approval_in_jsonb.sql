-- Jobcard approved via upsert_kernel_job_card only (RBAC already grants this function).
-- Approval when p_jobcard_approved is stripped by proxy: job_card_data.jobcard_approved = true.
-- Drop legacy 3-arg overload (PostgREST/proxy often hit it; it returned success without jobcard_approved).
...
BEGIN
    ...
    v_job_card_store := p_job_card_data - 'jobcard_approved';
