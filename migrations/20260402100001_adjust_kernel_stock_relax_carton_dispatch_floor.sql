-- Manual stock adjust: allow carton corrections to go below recorded dispatch cartons for a style.
-- Dispatch lines often omit or mismatch carton counts; kg vs dispatch is still enforced below.
-- Remaining cartons in get_kernel_batches use GREATEST(0, packed - dispatched) so UI stays non-negative.
...
COMMENT ON FUNCTION public.adjust_kernel_stock_on_hand(uuid, varchar, numeric, numeric, text) IS
    'Manual kernel stock adjust: enforces kg vs dispatched; carton deltas allowed for corrections (remaining cartons clamped in listings).';
