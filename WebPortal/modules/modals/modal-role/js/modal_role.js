...
var _modal_role = (function () {
    ...
    function bindAdvancedAccordionOnce() {
        var permCollapse = document.getElementById('roleModalAdvancedPermsCollapse');
        if (!permCollapse || permCollapse.dataset.roleModalPermBound === '1') return;
        permCollapse.dataset.roleModalPermBound = '1';
        permCollapse.addEventListener('shown.bs.collapse', function () {
            var form = document.getElementById('roleForm');
            var rid = form && form.getAttribute('data-editing-id');
            if (!rid) return;
            if (typeof _adminGrid !== 'undefined' && _adminGrid.loadAdminPermissions) {
                _adminGrid.selectedRoleId = rid;
                _adminGrid.adminPermCurrentPage = 1;
                _adminGrid.loadAdminPermissions(rid);
            } else {
                var tbody = document.getElementById('adminPermissionsTableBody');
                if (tbody) {
                    tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-3">Use <strong>User &amp; access</strong> for database permissions in the role editor, or open <strong>Role permissions</strong> from the menu.</td></tr>';
                }
            }
        });
    }
    ...
    var api = {
        init: function () {
            ...
            if (modalEl && typeof $ !== 'undefined') {
                $(modalEl).off('hidden.bs.modal.roleClear').on('hidden.bs.modal.roleClear', function () {
                    if (typeof _adminGrid !== 'undefined' && _adminGrid.onRoleModalHidden) _adminGrid.onRoleModalHidden();
                    api.clearForm();
                });
            }
        },
        ...
        show: function (role) {
            ...
            var wrap = document.getElementById('roleModalAdvancedAccordionWrap');
            if (wrap) {
                wrap.classList.toggle('d-none', !role || !role.id);
            }
            var toolbar = document.getElementById('roleModalAdvancedPermToolbar');
            if (toolbar) {
                toolbar.classList.toggle('d-none', typeof _adminGrid === 'undefined');
            }
            var collapseEl = document.getElementById('roleModalAdvancedPermsCollapse');
            if (collapseEl && typeof bootstrap !== 'undefined' && bootstrap.Collapse) {
                var coll = bootstrap.Collapse.getOrCreateInstance(collapseEl, { toggle: false });
                coll.hide();
            }
            var tbody = document.getElementById('adminPermissionsTableBody');
            if (tbody && role && role.id) {
            ...
            if (typeof _adminGrid !== 'undefined' && role && role.id) {
                _adminGrid.selectedRoleId = role.id;
            }
