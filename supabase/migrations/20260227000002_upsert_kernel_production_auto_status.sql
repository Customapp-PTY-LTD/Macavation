-- Migration: upsert_kernel_production — auto-transition intake/receiving → production
-- When any production data is saved for a batch still in intake or receiving status,
-- automatically move it to 'production' so it appears correctly in the production grid.
...
BEGIN
    -- Load current arrays and status
    SELECT
    ...
    SET
        ...
        status              = CASE
                                   -- Auto-promote intake/receiving → production when production data is saved
                                   WHEN v_has_production_data AND status IN ('intake', 'receiving')
                                        THEN 'production'::varchar
                                   -- Finish production: production → qa
...
-- RBAC: already granted from earlier migration — no new grant needed.
