...
**Use `sendTemplate` (`_shared/wa-send.ts:376`). Do not build the payload inline** — one owner for
the Control Room envelope is the point, and `sendViaControlRoom` (`_shared/wa-send.ts:295`) is it.
...
**The RPCs below are already applied on `dev`** — an earlier version of this plan said they were
missing, which is no longer true. You still cannot call them from here, so the contracts in this
plan are what you build against. **Do not add a fallback that reads the tables directly, and do not
degrade to guessing figures** — if an RPC ever is missing, the function should fail loudly with the
Postgres error so whoever deploys it finds out immediately.
...
registered in `package.json`: `scripts/verify-wa-plumbing.mjs` already covers the payload shapes
this function relies on, and `package.json` is edited by a different plan in this batch, so leave it
alone entirely.
...
still needs Meta's approval — do not describe either as outstanding work of yours.
