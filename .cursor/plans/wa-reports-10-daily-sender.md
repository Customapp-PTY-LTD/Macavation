...
`_shared/wa-send.ts` is merged and exports `sendTemplate`. **Use it. Do not build the payload
inline** — one owner for the Control Room envelope is the whole point.
...
**The RPCs below are already applied on `dev`** — that changed since the first version of this
plan, which told you they were missing. You still cannot call them, so treat the contracts in this
plan as the source of truth and do not try to confirm them. **Do not add a fallback that reads the
tables directly, and do not degrade to guessing figures** — if an RPC ever is missing the function
should fail loudly with the Postgres error, so whoever deploys it finds out immediately.
