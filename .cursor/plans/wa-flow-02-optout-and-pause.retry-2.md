...
**This is a hand-corrected third attempt.** The original run and the fleet's own auto-amended
retry (`wa-flow-02-optout-and-pause.retry-1.md`) were both blocked. Both blocking reports were read
in full and independently re-checked against this checkout before writing this version -- five
issues, confirmed real, are fixed below and are called out at the point each is fixed rather than
silently folded in, so the next reader does not have to diff three files to find out what changed
and why.
...
| `report_daily_recipients()` | `:260-277` | The recipient selector `send-daily-production-report` uses. **Not the only one that exists** -- see the corrected sender inventory below. |
...
**Corrected inventory.** The previous attempt claimed `report_daily_recipients()` was the only
recipient selector any sender uses. It is not: `send-daily-digest-whatsapp/index.ts:93-97` reads
`scheduled_reports` directly with `supabase.from('scheduled_reports').select('id, phone,
email').eq('is_active', true).eq('channel', 'whatsapp')` and posts straight to `meta-proxy` --
entirely outside `report_recipients`, `opted_out_at`, or any RPC this plan touches. Three senders
must be gated, not two:

| File | What it sends | Auth | Recipients from |
|---|---|---|---|
...
| `supabase/functions/send-report-whatsapp/index.ts` | A published report PDF, to **an arbitrary recipient list posted by the browser** | Portal session plus `has_action(user,'reports.report.send')` at `~:224` | Whatever the browser posts -- gated by deliverable 3 |
| `supabase/functions/send-daily-digest-whatsapp/index.ts` | A plain-text digest | Same service-role-bearer shape as the daily production report (read it, do not assume) | `scheduled_reports`, direct table read, no gate today -- gated by deliverable 3b |

The third is not scheduled anywhere in this repo today -- its own header says so ("NOT SCHEDULED
... nothing has ever been sent from here") -- and this plan does not change that, does not
investigate the `docs/phase2/*` documents that still describe a schedule for it, and does not
assert whether it is otherwise reachable. **What this plan cannot leave alone is the STOP reply's
promise.** If STOP only gates two of the three functions that can send a report-shaped WhatsApp
message, the words "you will not receive any further report messages" are false for as long as
this third function exists and is ever invoked -- by hand, by a future schedule, or by a mistake.
Deliverable 3b closes it. `send-report-whatsapp`'s per-recipient loop is at `~:380-513`;
`begin_report_delivery` is the first call inside it at `~:392`.
...
- A header comment in this repo's migration style: what was wrong, what this changes, and an
  ...
  not re-derive them.
- **Also record, as its own paragraph, a pre-existing exposure this migration's own design makes
  worse — flag it for a human, do not fix it here.** `list_report_distribution(boolean)` is
  granted to `anon` (`20260825090000…sql:394`) and returns `rr.phone` for every row, including
  inactive ones when called with `p_include_inactive=true`. Before this migration, that grant only
  ever exposed the phone numbers of people someone had deliberately added to the roster. After
  deliverable 1b, **a number that has never been anything but a stranger who once texted STOP now
  gets a roster row too** (`is_active=false`, real phone number) — so `list_report_distribution`
  becomes newly capable of enumerating the phone numbers of people who contacted this business
  only to ask to be left alone, to anyone holding the committed anon key. This migration must not
  change that grant — that is `list_report_recipients`'s and the portal's concern, out of scope
  here — but it must say so plainly, so whoever reviews this migration sees it named rather than
...
- `STOP`:
  ...
  - on success, `logCommand(sb, { phone: from, userId: null, wamid, rawBody, command: 'STOP',
    ...
    how to reverse it. **Scope the promise to what this plan actually gates: reports.** Do not
    say "you will not receive any further messages" (unscoped) or anything implying every kind
    of WhatsApp message from this number is covered — alerts and staff-menu replies are not
    touched by this plan and must not be promised silent by it. Word it so it is true for a
    number that was never on any list, e.g. "You will not receive any further report messages
    from Macavation. Text START if you want to allow them again." **Do not promise
    re-subscription** — clearing `opted_out_at` does not create a subscription;
...
`_shared` or `whatsapp-inbound/index.ts`.** Confirm this by reading this file's own `import`
lines at the top and confirming neither `../_shared/*` nor `../whatsapp-inbound/*` appears. The
previous attempt described "the same condition `isMissingRpc` detects" without saying this
plainly, and the function that got written called `isMissingRpc` as though it were in scope --
it is not, and that reference does not compile. Declare a small local function in this file, named
`isMissingRpcError`, with the identical detection logic `whatsapp-inbound/index.ts:184-188` uses
(read it and match its condition -- code `PGRST202`, or a message matching
`could not find the function|does not exist`) -- a second, independent declaration of the same
check, not an import, because there is nothing to import from.

- Immediately **before** the `begin_report_delivery` call (`~:392`), for the loop current
...
- The existing `rpc()` helper (`~:83-89`) **throws** rather than returning an error object -- read
  it before choosing how to call `report_opt_out_status`. Because it throws, wrapping the call in
  the loop existing `try` (`~:391`) and branching in a single `catch` would collapse the
  missing-RPC case and every other failure into one path -- exactly the inversion contract 3 below
  forbids. Write the two outcomes as two distinct `catch` arms (or an explicit `if` on the caught
  error before falling into a generic handler), not one shared block:
  ...
  try {
    ...
    if (isMissingRpcError(e)) {
      ...
      // failed delivery shape an explicit opt-out refusal would use, and skip this recipient.
      // <write the begin_report_delivery / complete_report_delivery(p_status:'failed') pair here,
      //  with an error naming the failed opt-out check, then push to results and continue>
      continue;
    }
  }
  if (optOut?.opted_out === true) {
    // <write the begin_report_delivery / complete_report_delivery(p_status:'failed') pair here,
    //  with p_error 'Recipient has opted out of WhatsApp report messages.', push to results,
    //  failed++, and continue -- do NOT send>
  }
  ```

  This skeleton is the contract, not decoration: the two `continue` paths must remain textually
  distinct arms (fail-open on `isMissingRpcError`, fail-closed on everything else), because
  deliverable 4 verifier asserts they are written as separate branches, not merged behind one
  `catch`.
- If `opted_out === true`: write the refusal through the same two RPCs the rest of the loop uses,
  so the delivery history stays complete -- `begin_report_delivery` with the same arguments the
  send path passes, then `complete_report_delivery` with `p_status: 'failed'` and a `p_error` that
  names opt-out. **Do not send.** Push a `results` entry with `status: 'failed'` and that error,
  `failed++`, and `continue`. The whole request must still return 200 with per-recipient detail
  (`~:515-519`).
- Do not add `replace(/\D/g` anywhere new in this file (the existing `normalizePhone` at
  `~:148-153` is inventory row 3 and must stay byte-identical -- `report-whatsapp-parity:verify`
  asserts its exact literal).

### 3b. `send-daily-digest-whatsapp` -- gate the third sender the previous attempt missed

**New in this version.** `supabase/functions/send-daily-digest-whatsapp/index.ts:93-97` reads
`scheduled_reports` directly and sends to every row with `is_active=true, channel='whatsapp'`,
entirely outside `report_recipients`. It is not scheduled anywhere in this repo (its own header
says so), but it exists, it is deployed, and it can be invoked by hand -- so the STOP reply
promise is false for as long as it is ungated. This deliverable makes the promise true for every
sender that exists today, without touching whether this function is ever actually scheduled --
that is a separate, stale-docs question this plan does not investigate or resolve.

Read the function loop (the `for (const sub of subs || [])` block, roughly `:92-125`) before
changing it. Same reasoning as deliverable 3, condensed because the loop is simpler and has no
`begin_report_delivery`/`complete_report_delivery` pair to preserve:

- Immediately inside the loop, before the `fetch` to `meta-proxy`, call
  `report_opt_out_status` with `{ p_phone: to }` (after `normalizePhone` has produced `to` --
  pass the normalised form; `report_opt_out_status` normalises again internally via
  `chat_normalize_phone`, and calling it twice with different input shapes is harmless because
  both ends of that comparison are idempotent, but confirm this by reading
  `chat_normalize_phone` own definition rather than assuming it).
- This file has no local `isMissingRpc`-equivalent either and does not import from
  `send-report-whatsapp` or `whatsapp-inbound`. Declare the same small local
  `isMissingRpcError` function here too, independently -- a third copy of an eleven-line
  detection check across three files that cannot see each other is the correct outcome, not a
  smell to refactor away in this plan.
- Same fail-open/fail-closed split as deliverable 3: missing RPC leads to `console.error` naming
  the migration and sending anyway; any other error means skip this recipient, do not send, and
  `console.error` the reason. There is no delivery-row pair to write here (this function has none
  today) -- a `console.error` and a `continue` is the whole of it. Do not add one; that would be
  scope this plan was not asked for.
- If `opted_out === true`: `continue` without incrementing `sent` and without calling
  `mark_scheduled_report_sent` -- an opted-out row must not be marked sent, since it was not.
...
6. The `STOP` path is reachable for an unenrolled number. **The previous attempt's ordering
   check used a whole-file `indexOf` and was unsound**: `outcome: 'not_enrolled'` appears TWICE in
   this file -- once inside `tryConfirmEnrolment` (the six-digit-code failure path) and once inside
   `processCommandForMessage` (the reply this assertion actually cares about). A bare
   `indexOf("outcome: 'not_enrolled'")` returns the FIRST occurrence, which sits inside
   `tryConfirmEnrolment` and is defined earlier in the file than `processCommandForMessage` itself --
   so the comparison is false regardless of where the interceptor call actually sits, and the check
   can never catch a real regression. Fix, in this order:
   a. **Isolate `processCommandForMessage`'s own body first**, using the exact brace-depth-counting
      idiom `scripts/verify-wa-role-features.mjs` already uses for `loadFeatureKeysBody()` (find
      `async function loadFeatureKeysBody` there and copy its walk-to-matching-close-brace approach,
      not just its name) -- locate `async function processCommandForMessage`, then walk forward
      counting `{`/`}` from its first `{` to the brace that returns depth to zero. Call the result
      `fnBody`.
   b. **Add an isolation sanity check before trusting `fnBody` for anything**: assert
      `(fnBody.match(/outcome: 'not_enrolled'/g) || []).length === 1`. If that count is ever not
      exactly 1 -- the isolation boundary drifted, or a second occurrence was added inside the same
      function -- fail loudly naming what changed, rather than silently comparing against the wrong
      occurrence again the way the previous attempt did.
   c. **Do the three-way ordering comparison entirely within `fnBody`, not the whole file**:
      `fnBody.indexOf('handleOptOutVerbs(sb, from, wamid, rawBody, replyId)')` must be greater than
      −1 and less than `fnBody.indexOf("sb.rpc('whatsapp_resolve_staff_user'")`, which must in turn
      be less than `fnBody.indexOf("outcome: 'not_enrolled'")`. Within the isolated body this is sound,
      because step b already proved there is exactly one occurrence to compare against.
   d. State in a comment that source order is what proves it here (this script never executes
      `.ts`), that the interceptor sits inside `processCommandForMessage`'s existing `try`, and that a
      `COMMAND_HANDLERS.STOP` entry is deliberately absent because `CommandContext` does not exist
      before resolution.
   **Do not loosen, reorder, or remove any of steps a-d to make this assertion pass.** If a future
   change to `whatsapp-inbound/index.ts` makes this check fail, the fix is to correct that change --
   never to weaken the check, re-scope it back to the whole file, or drop the isolation sanity check
   in step b.
...
9a. `send-report-whatsapp/index.ts` declares its own `function isMissingRpcError` (or a `const`
   arrow of the same name — check for either form) — **not** a bare call to `isMissingRpc` with no
   matching local or imported declaration in this file. This is the specific bug that blocked the
   previous two attempts: assert the declaration exists, not just that the name appears somewhere,
   so a stray reference to an undeclared `isMissingRpc` cannot pass by accident.
9b. The two `catch`/`if` arms deliverable 3's skeleton describes are textually distinct in the
   built file — assert that `isMissingRpcError(e)` (or however the file spells the check) appears
   as a condition, and that there are **two** separate `continue` statements reachable from the
   opt-out check block, not one. This is the guard against the fail-open and fail-closed branches
   being silently collapsed into a single generic `catch`.
9c. `supabase/functions/send-daily-digest-whatsapp/index.ts` calls `report_opt_out_status`, and
   its first occurrence appears **before** the first `fetch(\`${CONTROL_ROOM_BASE_URL}/meta-proxy\`)`
   occurrence in that file. Also assert this file declares its own local `isMissingRpcError` (same
   check as 9a, independent file) rather than importing one that does not exist anywhere to import
   from.
...
**Before you finish, prove the verifier bites.** For at least assertions 1, 3, 4, 6 and 9c, break
the thing deliberately, run the verifier, confirm it fails, restore it, confirm it passes. Say in
your report what you broke and quote the exact failure message each time — assertion 6 (the
ordering check) and assertion 9c (the digest sender's gate) are the two that blocked this plan
twice already, so give them the most scrutiny: for assertion 6, specifically prove that moving the
interceptor call to AFTER the `not_enrolled` reply inside `processCommandForMessage` makes it fail,
not just that removing the interceptor entirely makes it fail — those are different regressions
and a check that only catches the second one is not proving the ordering property at all. A
verifier that cannot fail is worse than none, because it converts an unverified change into a
green tick.
