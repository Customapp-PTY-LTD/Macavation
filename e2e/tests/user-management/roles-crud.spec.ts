...
test.describe('Roles Management - CRUD Operations @roles-management', () => {
  ...
  test('TC-RM-005: Search Roles', async ({ authenticatedPage }) => {
    ...
    await authenticatedPage.click('[onclick*="search"]');
  ...
  test('TC-RM-006: Filter Roles by Permission Level', async ({ authenticatedPage }) => {
    /**
     * Test permission level filter
     */
    ...
    const levelFilter = authenticatedPage.locator('#filterLevel');
    const options = await levelFilter.locator('option').allTextContents();
    expect(options.length).toBeGreaterThan(1);
  });

  test('TC-RM-007: Filter Roles by Status', async ({ authenticatedPage }) => {
    /**
     * Test status filter
     */
    const filterToggle = authenticatedPage.locator('[data-bs-target="#filtersCollapse"]');
    ...
    const statusFilter = authenticatedPage.locator('#filterStatus');
