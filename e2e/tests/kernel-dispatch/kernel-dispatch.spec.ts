 ...
 *
 * Per QA_STRATEGY_BLUEPRINT: Grid View, View Toggle, Refresh, optional modal (View basket).
 * Kernel Dispatch lists orders (baskets) sent from Stock (Kernel). Board and Table views.
 */
...
test.describe('Kernel Dispatch @kernel-dispatch', () => {
  ...
  test('TC-KD-005: Table view shows dispatch orders columns', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.locator('th:has-text("Buyer")')).toBeVisible();
    await expect(authenticatedPage.locator('th:has-text("Delivery date"), th:has-text("Created")').first()).toBeVisible();
  });
  ...
  test('TC-KD-006: Refresh button visible and clickable', async ({ authenticatedPage }) => {
    ...
    await authenticatedPage.waitForLoadState('networkidle');
    await expect(authenticatedPage.locator('#kdKanbanBoard, #kdTableCards')).toBeVisible({ timeout: 5000 });
  });
