...
**Ensuring all users can use the app:** To avoid 403 errors for any authenticated user, ensure **all roles** have EXECUTE on data functions. Apply the migration **`20260218000001_grant_all_data_functions_to_all_roles.sql`** (see `docs/RBAC_GUIDE.md` and `migrations/20260218000001_grant_all_data_functions_to_all_roles.sql`). That migration grants EXECUTE on all app data functions to every role in the `roles` table, so all users have access to data functions regardless of role.
